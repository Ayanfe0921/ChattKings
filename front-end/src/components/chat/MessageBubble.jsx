...
import { CornerUpLeftIcon, SmilePlusIcon } from "lucide-react";
...
export function MessageBubble({ message, onReply, onReact, currentUserId }) {
    ...
    <div className={`flex w-full ${isOwnMessage ? "justify-end" : "justify-start"}`}>
      ...
      >
        ...
        <div className="mt-1 flex items-center justify-end gap-1">
          <button type="button" onClick={() => onReply(message)} className="grid size-7 place-items-center rounded-full opacity-70 hover:bg-black/10 hover:opacity-100" aria-label="Reply to message" title="Reply">
            <CornerUpLeftIcon className="size-4" />
          </button>
          <div className="relative">
            <button type="button" onClick={() => setShowReactions((open) => !open)} className="grid size-7 place-items-center rounded-full opacity-70 hover:bg-black/10 hover:opacity-100" aria-label="React to message" title="React">
