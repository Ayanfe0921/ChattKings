  ...
  return (
    <div className={`flex w-full ${isOwnMessage ? "justify-end" : "justify-start"}`}>
      ...
      >
        ...
        {message.postReply ? <div className={`mb-2 max-w-64 overflow-hidden rounded-xl border-l-2 ${isOwnMessage ? "border-white/70 bg-black/10" : "border-accent bg-background/70"}`}><div className="flex items-center gap-2 p-2"><div className="min-w-0 flex-1"><p className="text-[10px] font-semibold opacity-80">{message.postReply.authorName || "Post"} · {message.postReply.mediaType === "video" ? "Video post" : message.postReply.mediaType === "quote" ? "Quote post" : "Photo post"}</p><p className="mt-0.5 line-clamp-2 text-xs">{message.postReply.mediaType === "quote" ? message.postReply.quoteAuthor || "Quote post" : message.postReply.caption || "Post"}</p></div>{message.postReply.mediaUrl && message.postReply.mediaType === "image" ? <img src={message.postReply.mediaUrl} alt="" className="size-12 shrink-0 rounded-lg object-cover" /> : message.postReply.mediaUrl && message.postReply.mediaType === "video" ? <video src={message.postReply.mediaUrl} muted playsInline preload="metadata" className="size-12 shrink-0 rounded-lg object-cover" /> : null}</div>{message.postReply.mediaType === "quote" && message.postReply.quoteText ? <p className="border-t border-border/50 px-2 py-1.5 text-xs italic">“{message.postReply.quoteText}”</p> : null}</div> : null}
